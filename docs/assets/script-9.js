...
(() => {
  ...
  const mount = () => {
    ...
    if (!input.dataset.chengmoSearchReady) { input.dataset.chengmoSearchReady = 'true'; input.addEventListener('input', scheduleSync); input.addEventListener('keydown', event => { if (event.key === 'Escape' && input.value) { event.preventDefault(); reset(input); } }); }
    sync();
  };
